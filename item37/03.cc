...
class point_avg : public std::binary_function<point, point, point>
{
public:
  point_avg() : num_(0), xsum_(0.0), ysum_(0.0) { }
  point operator() (const point &avg, const point &pt)
  {
    ++ num_; 
    xsum_ += pt.x_; 
    ysum_ += pt.y_; 
    return point(xsum_/num_, ysum_/num_); 
  }
...
int main()
{
  ...
  cout << accumulate(ptvec.begin(), ptvec.end(), point(), point_avg()) << endl; 
